...
// Strip commas and whitespace before numeric check
// Handles Excel-formatted values like " 2,476 " or "1,23,456"
function cleanNumeric(v) {
  if (v === null || v === undefined || v === '') return '';
  return String(v).trim().replace(/,/g, '');
}
function isNumeric(v) {
  const cleaned = cleanNumeric(v);
  if (cleaned === '') return false;
  return !isNaN(Number(cleaned));
}
function toNumber(v) {
  return Number(cleanNumeric(v));
}
...
function validateData(rows, mapping) {
  ...
  numFields.forEach(f => {
    const col = mapping[f]; if (!col) return;
    rows.forEach((r, i) => { const v = toNumber(r[col]); if (!isNaN(v) && v < 0) negRows.push({ row: i + 2, field: f, value: v }); });
  });
  if (negRows.length > 0)
  ...
  if (paidCol && repCol)
    rows.forEach((r, i) => {
      const p = toNumber(r[paidCol]), rep = toNumber(r[repCol]);
      if (!isNaN(p) && !isNaN(rep) && p > rep) paidOverReported.push({ row: i + 2, paid: p, reported: rep });
    });
  if (paidOverReported.length > 0)
    warnings.push({ check: 'paid_exceeds_reported', message: 'Paid > Reported in ' + paidOverReported.length + ' row(s) — review recommended but data will still be saved', detail: paidOverReported.slice(0, 10) });
  ...
  const delayWarn = [];
  if (delayCol) rows.forEach((r, i) => { const v = toNumber(r[delayCol]); if (!isNaN(v) && (v < 0 || v > 500)) delayWarn.push({ row: i + 2, delay: v }); });
