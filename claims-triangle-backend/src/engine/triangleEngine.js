...
function getMetricValue(row, mapping, metric) {
  if (metric === 'paid')     return Number(row[col(mapping,'Paid_Amount')]     || 0);
  if (metric === 'reported') return Number(row[col(mapping,'Reported_Amount')] || 0);
  if (metric === 'count')    return Number(row[col(mapping,'Reported_Count')]  || 0);
  return 0;
}
